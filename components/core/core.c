...
int16_t minorOf3x3Matrix16(uint8_t row, uint8_t column, Matrix16_t * matrix)
{
    ...
    for(uint16_t rowIndex = 0; rowIndex < matrix->numOfRows; rowIndex++){
        ...
        for(uint16_t columnIndex = 0; columnIndex < matrix->numOfCols; columnIndex++)
        {
...
void scaledInverse3x3Matrix16(Matrix16_t * matrix, Matrix16_t  * matrixInverse)
{
    ...
    for(uint16_t rowIndex = 0; rowIndex < matrix->numOfRows; rowIndex++)
    {
        for(uint16_t columnIndex = 0; columnIndex < matrix->numOfCols; columnIndex++)
        {
            cofactorMatrix.m[rowIndex][columnIndex] = INVERSE_MATRIX_SCALER * signMatrix.m[rowIndex][columnIndex] * minorOf3x3Matrix16(rowIndex, columnIndex, matrix) / determinant;
        }
    }
